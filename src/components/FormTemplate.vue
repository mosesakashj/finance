        ...
        <v-layout wrap class="pa-3">
          ...
          <v-flex :class="prop.class" v-for="(prop, index) in references.properties" :key="index" :style="prop.style">
            <template v-if="prop.type === formType.TEXT">
              <v-text-field outlined :hide-details="prop['hide-details']" :label="prop.label" :suffix="prop.suffix" :id="prop.model" :ref="prop.ref" @input="prop.input" :disabled="prop.disabled" :rules="prop.rules" v-model="model[prop.model]"> </v-text-field>
            </template>
            <template v-if="prop.type === formType.NUMBER">
              <v-text-field outlined :hide-details="prop['hide-details']" :type="formType.NUMBER" :id="prop.model" :label="prop.label" :rules="prop.rules" v-model="model[prop.model]" :disabled="prop.disabled"> </v-text-field>
            </template>
            <template v-else-if="prop.type === formType.TEXTAREA">
              <v-textarea outlined :hide-details="prop['hide-details']" :label="prop.label" v-if="!prop.is_show" :id="prop.model" :rules="prop.rules" v-model="model[prop.model]" :disabled="prop.disabled" :rows="prop.rows"></v-textarea>
            </template>
            ...
            <template v-else-if="prop.type === formType.DATEPICKER">
              <v-menu :close-on-content-click="false" v-model="model[prop.menu]" transition="scale-transition" offset-y min-width="290px" v-if="prop.is_show !== undefined ? prop.is_show : true">
                <template v-slot:activator="{ on }">
                  <v-text-field outlined hide-details v-model="model[prop.picker]" :rules="prop.rules" clearable :label="prop.label" prepend-icon="mdi-calendar"
                  readonly v-on="on" :disabled="prop.disabled" @click:clear="$nextTick(() => model[prop.picker] = null)" @change="prop.change"></v-text-field>
                </template>
            ...
            <template v-else-if="prop.type === formType.FILES && prop.is_show">
              <v-file-input :accept="prop.accept" :disabled="prop.disabled" outlined v-model="model[prop.model]" :rules="prop.rules" :label="prop.label" :multiple="prop.multiple" @change="prop.change"></v-file-input>
