          ...
          <template v-if="!payload.selection.length">
            <template class="mt-0">
              <v-flex sm8 xs4 v-if="!payload.hasFilters">
                <v-btn v-if="!payload.isHideAdd" color="primary" dark id="newbtn" @click="payload.addNewRoute">add</v-btn>
                <v-btn v-else-if="payload.addClickHandler" color="primary" dark id="newbtn" >add</v-btn>
              </v-flex>
  ...
  watch: {
    paginationMap (newValue) {
      if (!this.isEmptyObject(newValue)) {
        const result = Object.assign({ ...this.paginationMap }, { search: this.search })
        this.$emit('change', result)
      }
    },
    search (val) {
      const search = Object.assign({ ...this.paginationMap }, { search: val })
      this.$emit('change', search)
    },
    'payload.headers' (val) {
      // val
      const booleanList = this.payload.headers.filter(x => x.isBoolean)
      this.booleanItems = booleanList.map(x => x.value)
    }
    // 'payload.selection' () {
    //   console.log(this.payload.selection);
    // }
  },
  methods: {
    deleteEntries () {
      const ids = this.payload.selection.map(x => x.id)
      const data = { module: this.payload.module, ids }
